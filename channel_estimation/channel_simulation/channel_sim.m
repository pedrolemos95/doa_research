...
scores = cellfun(@(los_cand) los_candidates{1}(end), los_candidates);
...
% Estimated dirs
estimated_dirs = cellfun(@(param) dir_fun(param), los_candidates, 'UniformOutput', false);

% Estimated positions
dz = rx_height - tx_height;
pos_estimate_fun = @(mu) rx_pos + [0;0;tx_height - rx_height] + (dz/(k*sqrt(1 - (mu(2)^2 + mu(3)^2)/k^2)))*[mu(2);mu(3);0];
...
dir_error = arrayfun(@(n) (180/pi)*acos(estimated_dirs{n}.'*real_dirs{n}/(norm(estimated_dirs{n})*norm(real_dirs{n}))), 1:num_observations);
