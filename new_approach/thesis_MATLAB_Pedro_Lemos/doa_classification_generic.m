function results = doa_classification_generic(K_high, K_low, dim_input, num_observations)
    ...
    % Aperture dimensions
    M_1 = dim_input(1); % frequency related
    M_2 = dim_input(2); % spatial related
    M_3 = dim_input(3); % spatial related
    ...
    % h.SizeData = 300;
    % scatter(tx_pos(1),tx_pos(2),'filled'); % transmitter position
    % h.SizeData = 300;
    ...
    results("class_dir_error") = dir_errors_cleaned;
    results("los_ests") = los_est;
    results("est_ks") = est_k;
    %% Generate figures
