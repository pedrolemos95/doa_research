...
static uint8_t activeThreadsUsingUART = 0;
static HANDLE  serialHandles[MAX_HANDLE_NUM];
...
static void   saveSerialHandle(HANDLE handle);

static HANDLE getSerialHandle();
...
{
  return (uartCloseSerial(getSerialHandle()) == 0) ? -1 : 0;
}
...
int32_t uartRx(uint32_t dataLength, uint8_t* data)
{
  ...
  if (getSerialHandle() == INVALID_HANDLE_VALUE) {
    return -1;
  }

  while (dataToRead) {
    ret = ReadFile(getSerialHandle(), (LPVOID)data, dataToRead, (LPDWORD)&dataRead, NULL);
    if (!ret) {
      ret = GetLastError();
...
int32_t uartRxNonBlocking(uint32_t dataLength, uint8_t* data)
{
  ...
  ret = ReadFile(getSerialHandle(), (LPVOID)data, (DWORD)dataLength, (LPDWORD)&dataRead, NULL);
  if (!ret) {
    ret = GetLastError();
...
int32_t uartRxPeek(void)
{
  ...
  if (!ClearCommError(getSerialHandle(), &err, &comStat) || err) {
    return -1;
  }
...
int32_t uartTx(uint32_t dataLength, uint8_t* data)
{
  ...
  while (dataToWrite) {
    ret = WriteFile(getSerialHandle(), (LPCVOID)data, dataToWrite, (LPDWORD)&dataWritten, NULL);
    if (!ret) {
      return -1;
    }
  ...
  FlushFileBuffers(getSerialHandle());
                             ...
                             uint32_t stopBits, uint32_t rtsCts, uint32_t xOnXOff, int32_t timeout)
{
  ...
  if (!GetCommState(serial, &settings)) {
    fprintf(stderr, "Error getting comm state on %s - %d.\n",
            (char*)device,
            GetLastError());
    goto error;
  }
  ...
  if (!SetCommState(serial, &settings)) {
    fprintf(stderr, "Error setting comm state on %s - %d.\n",
            (char*)device,
            GetLastError());
    goto error;
  }
  ...
  if (!SetCommTimeouts(serial, &commTimeouts)) {
    fprintf(stderr, "Error setting comm timeouts on %s - %d.\n",
            (char*)device,
            GetLastError());
    goto error;
  }
...
static int32_t uartCloseSerial(HANDLE handle)
{
  ...
  if (!(ret = CloseHandle(handle))) {
    fprintf(stderr, "Error closing serial port - %d.\n", GetLastError());
  }
...
static void saveSerialHandle(HANDLE handle)
{
  ...
  for (int i = 0; i < activeThreadsUsingUART; i++) {
    if (threadIDs[i] == threadID) {
      serialHandles[i] = handle;
      return;
    }
  }

  threadIDs[activeThreadsUsingUART] = threadID;
  serialHandles[activeThreadsUsingUART] = handle;
  activeThreadsUsingUART++;
}
