...
static int32_t    serialHandles[MAX_HANDLE_NUM];
...
static void    saveSerialHandle(int32_t handle);

static int32_t getSerialHandle();
...
{
  return uartCloseSerial(getSerialHandle());
}
...
int32_t uartRx(uint32_t dataLength, uint8_t* data)
{
  ...
  if (getSerialHandle() == -1) {
    return -1;
  }

  while (dataToRead) {
    dataRead = read(getSerialHandle(), (void*)data, dataToRead);
    if (-1 == dataRead) {
      return -1;
    } else {
      dataToRead -= dataRead;
...
int32_t uartRxNonBlocking(uint32_t dataLength, uint8_t* data)
{
  ...
  dataRead = read(getSerialHandle(), (void*)data, (size_t)dataLength);
  if (-1 == dataRead) {
    return -1;
  }
...
int32_t uartRxPeek(void)
{
  ...
  if (-1 == ioctl(getSerialHandle(), FIONREAD, (int*)&bytesInBuf)) {
    return -1;
  }
...
int32_t uartTx(uint32_t dataLength, uint8_t* data)
{
  ...
  while (dataToWrite) {
    dataWritten = write(getSerialHandle(), (void*)data, dataToWrite);
    if (-1 == dataWritten) {
      if (EAGAIN == errno) {
        continue;
      } else {
